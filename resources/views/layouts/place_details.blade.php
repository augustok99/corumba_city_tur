    ...
    <div class="flex flex-col items-center justify-center">
        ...
        <ul class="mt-2 flex justify-center items-center flex-col">
            ...
            <a href="{{ $place->links}}" target="_blank"
                class="text-lg text-indigo-600 font-poppins mt-1">{{$place->links ?? 'Link não disponível' }}</a>
        </ul>
        @endif

        <div class="flex flex-col justify-center items-center">
            <span class="text-lg font-semibold mt-2 font-poppins">Descrição do Local:</span>
            <p class="text-lg font-poppins mt-2">{{ $place->description ?? 'Descrição não disponível' }}</p>
        </div>
    </div>
